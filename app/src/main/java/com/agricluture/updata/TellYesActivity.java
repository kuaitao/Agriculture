...
public class TellYesActivity extends BaseActivity {

    protected List<Node> mDatas = new ArrayList<Node>();
    @BindView(R.id.recyclerview)
    RecyclerView recyclerview;
    @BindView(R.id.rly_bottom_btn)
    RelativeLayout rlyBottomBtn;
    ...
    protected void initViews() {
        classflags = getIntent().getIntExtra("flag", 0);
        if (classflags == 2) {
            initTitleBar(R.string.search_reault);
            tvYib.setVisibility(View.GONE);
        } else {
            initTitleBar(R.string.tell_yes);
            tvYib.setVisibility(View.VISIBLE);
        }

        rlyBottomBtn.setVisibility(View.GONE);
        ...
        mAdapter = new SimpleTreeRecyclerAdapter(recyclerview, activity,
                mDatas, 0, R.mipmap.more_up, R.mipmap.more_content_black, 1);
